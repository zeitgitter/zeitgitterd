...
# `<version>` is extracted from `zeitgitter/version.py`.
VERSIONMATCH = ==$(shell sed -n -e 's,^VERSION = .\([0-9]*.[0-9]*.[0-9]*\).*$$,\1,p' zeitgitter/version.py)
...
docker-dev:gen-docker-dev
	sudo docker build --tag zeitgitter:dev zeitgitter-dev
